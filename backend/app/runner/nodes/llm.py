...
from ..model_runtime import call_chat_model, effective_model_config, resolve_node_model, runtime_value
...
def execute_live(node: NodeIR, state: dict[str, Any], ctx: ExecutionContext):
    ...
    response = call_chat_model(provider, model, messages, resolved_model_config)
    content = getattr(response, "content", str(response))
    output_field = str(config.get("outputField", f"{node.id}_output"))
    config_name = runtime_value(ctx.model_config, "name")
